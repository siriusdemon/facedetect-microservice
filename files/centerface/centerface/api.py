...
def ensure_size(im):
    maxh = 1000
    maxw = int(maxh / im.shape[0] * im.shape[1])
    im = cv2.resize(im, (maxw, maxh))
    return im
...
def detect(im):
    ...
    # im = ensure_size(im)
    h, w = im.shape[:2]
    dets, lms = net(im, h, w, threshold=0.7)
    res = []
    for (det, lm) in zip(dets, lms):
        det = list(map(lambda x: round(float(x), 2), det))
        lm = list(map(lambda x: round(float(x), 2), lm))
        res.append([det, lm])
    return res


def align(im, tsize=128, desiredLeftEye=0.28):
    im = ensure_cv(im)
    res = detect(im)
    crop = []
    for (det, lm) in res:
